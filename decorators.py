...
def token_required(f):
    ...
    def decorated(*args, **kwargs):
        ...
        try:
            ...
            is_user_allowed = LoginInfo.query.filter(LoginInfo.email == current_user.email).first()
...
def admin_required(f):
    ...
    def decorated(*args, **kwargs):
        ...
        try:
            data = jwt.decode(token, SECERET_KEY, algorithms=["HS256"])
            current_user = Admin.query.filter(Admin.email == data['email']).first()
            is_admin_allowed = LoginInfo.query.filter(LoginInfo.email == current_user.email).first()
